                ...
                <nav class="breadcrumb mt-1 mb-0" aria-label="breadcrumbs">
                    <ul>
                        ...
                        <li class="is-size-7 has-text-weight-medium is-active"><a href="/dashboard"
                                aria-current="page">SourceRef</a></li>
                    </ul>
                </nav>
            </div>
        </div>
                ...
                <div class="card has-background-white has-text-black">
                     <header class="card-header">
                        ...
                        <p class="card-header-icon">
                         <NuxtLink to="/sourceref/create" class="button is-link has-background-primary">
                        Create New SourceRef</NuxtLink>
                        </p>
                      </header>
                ...
                <vue-good-table
                    ...
                    <template slot="table-row" slot-scope="props">
                        <span v-if="props.column.field == 'action'">
                            <NuxtLink :to="'sourceref/' + rows[props.row.originalIndex].id" class="button is-link has-background-primary">
                                Edit</NuxtLink>
                            <button @click="deleteSourceRef(rows[props.row.originalIndex].id)" class="button is-danger">
                                Delete</button>
                        </span>
...
export default {
        ...
        return {
            ...
            columns: [
                ...
                },
                {
                    ...
                    filterOptions: {
                        enabled: true, // enable filter for this column
                        placeholder: 'Filter Sour Id', // placeholder for filter input
                        filterValue: '', // initial populated value for this filter
                        filterDropdownItems: [], // dropdown (with selected values) instead of text input
                        filterFn: this.columnFilterFn, //custom filter function that
                    },
    ...
    methods: {
        ...
        async loadItems() {
            const response = await this.$axios.$get("/api/sourceref", {
