                ...
                <nav class="breadcrumb mt-1 mb-0" aria-label="breadcrumbs">
                    <ul>
                        ...
                        <li class="is-size-7 has-text-weight-medium is-active"><a href="/dashboard"
                                aria-current="page">Citation</a></li>
                    </ul>
                </nav>
            </div>
        </div>
                ...
                <div class="card has-background-white has-text-black">
                     <header class="card-header">
                        ...
                        <p class="card-header-icon">
                         <NuxtLink to="/citation/create" class="button is-link has-background-primary">
                        Create New Citation</NuxtLink>
                        </p>
                      </header>
                ...
                <vue-good-table
                    ...
                    <template slot="table-row" slot-scope="props">
                        <span v-if="props.column.field == 'action'">
                            <NuxtLink :to="'citation/' + rows[props.row.originalIndex].id" class="button is-link has-background-primary">
                                Edit</NuxtLink>
                            <button @click="deleteCitation(rows[props.row.originalIndex].id)" class="button is-danger">
                                Delete</button>
                        </span>
...
export default {
    ...
    data() {
        return {
            ...
            columns: [
                ...
                //     filterOptions: {
                //         enabled: true, // enable filter for this column
                //         placeholder: 'Filter Volume', // placeholder for filter input
                //         filterValue: '', // initial populated value for this filter
                //         filterDropdownItems: [], // dropdown (with selected values) instead of text input
                //         filterFn: this.columnFilterFn, //custom filter function that
                //     },
                ...
                },
                {
                    ...
                    filterOptions: {
                        enabled: true, // enable filter for this column
                        placeholder: 'Is Active', // placeholder for filter input
                        filterValue: '', // initial populated value for this filter
                        filterDropdownItems: [], // dropdown (with selected values) instead of text input
                        filterFn: this.columnFilterFn, //custom filter function that
                    },
                },
                {
                    ...
                    filterOptions: {
                        enabled: true, // enable filter for this column
                        placeholder: 'Confidence', // placeholder for filter input
                        filterValue: '', // initial populated value for this filter
                        filterDropdownItems: [], // dropdown (with selected values) instead of text input
                        filterFn: this.columnFilterFn, //custom filter function that
                    },
                },
                {
                    ...
                    filterOptions: {
                        enabled: true, // enable filter for this column
                        placeholder: 'Source Id', // placeholder for filter input
                        filterValue: '', // initial populated value for this filter
                        filterDropdownItems: [], // dropdown (with selected values) instead of text input
                        filterFn: this.columnFilterFn, //custom filter function that
                    },
    ...
    methods: {
        ...
        async loadItems() {
            const response = await this.$axios.$get("/api/citation", {
        ...
        deleteCitation(id) {
             if(confirm("Do you really want to delete?")){
               ...
               const response = this.$axios.$delete("/api/citation/" + id)
