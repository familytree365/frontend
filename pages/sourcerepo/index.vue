                ...
                <div class="card has-background-white has-text-black">
                     <header class="card-header">
                        ...
                        <p class="card-header-icon">
                         <NuxtLink to="/sourcerepo/create" class="button is-link has-background-primary">
                        Create New SourceRepo</NuxtLink>
                        </p>
                      </header>
                ...
                <vue-good-table
                    ...
                    <template slot="table-row" slot-scope="props">
                        <span v-if="props.column.field == 'action'">
                            <NuxtLink :to="'sourcerepo/' + rows[props.row.originalIndex].id" class="button is-link has-background-primary">
                                Edit</NuxtLink>
                            <button @click="deleteSourceRepo(rows[props.row.originalIndex].id)" class="button is-danger">
                                Delete</button>
                        </span>
...
export default {
        ...
        return {
            ...
            columns: [
                ...
                },
                {
                    ...
                    filterOptions: {
                        enabled: true, // enable filter for this column
                        placeholder: 'Filter Repo Id', // placeholder for filter input
                        filterValue: '', // initial populated value for this filter
                        filterDropdownItems: [], // dropdown (with selected values) instead of text input
                        filterFn: this.columnFilterFn, //custom filter function that
                    },
                },
                {
                    ...
                    filterOptions: {
                        enabled: true, // enable filter for this column
                        placeholder: 'Filter Caln', // placeholder for filter input
                        filterValue: '', // initial populated value for this filter
                        filterDropdownItems: [], // dropdown (with selected values) instead of text input
                        filterFn: this.columnFilterFn, //custom filter function that
                    },
    ...
    methods: {
        ...
        async loadItems() {
            const response = await this.$axios.$get("/api/sourcerepo", {
